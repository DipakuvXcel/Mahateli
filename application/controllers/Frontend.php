...
class Frontend extends CI_Controller {
	...
	/*public function __construct() {
		 ...
		 $shop_data1 = $this->user_model->get_common('about_shop_own', array('status'=>1), '*', '2', '', '',''); 
		 $global_data['shop_data'] = $shop_data1;
	...
	public function index()
	{
		...
		$table = 'brands';
		$where = array('status_id' => 1);
		$brands = $this->user_model->get_common($table, $where,'*',2);

		$table1 = 'blogs';
		$where1 = array('status_id' => 1); // 3- deleted
		$blogs = $this->user_model->get_common($table1, $where1,'*',2,3);
		
		$table2 = 'products';
		$where2 = array('status' => 1, 'free_flag' => 0); // 3- deleted
		$feat_product = $this->user_model->get_common($table2, $where2,'*',2,5,'','position');
		//$our_product = $this->user_model->get_common($table2, $where2,'*',2,3,'main_category','product_id','ASC');

        $table3 = 'video';
		$where3 = array('status_id' => 1); // 3- deleted
		$video_product = $this->user_model->get_common($table3, $where3,'*',2,5,'','id');
		 
		$table4 = 'testimonial';
		$where4 = array('status_id' => 1); // 3- deleted
		$testimonial = $this->user_model->get_common($table4, $where4,'*',2,3,'','id');

		$data = array(
			'page_title'=>"MyFuel | Home", 
			'active'=>'home', 
	...
	public function login(){
		...
		$data = array('page_title'=>"MyFuel | Login", 'active'=>'login' );
	...
	public function registration(){
		...
		$data = array('page_title'=>"MyFuel | Registration", 'active'=>'registration' );
	...
	function save_registration(){
		...
		$this->form_validation->set_rules ( 'contact', 'Mobile', 'required|numeric|min_length[10]|max_length[10]|is_unique[user.contact]');
		...
		if($this->form_validation->run()==false){
			...
			$message = "<div style='padding: 30px; font-family: 'Helvetica Neue', 'Helvetica', 'Arial', sans-serif; line-height: 25px; 
				...
				Dear ".$name.",
				<p>
					Welcome! Thank you to register with us.<br>
					You are just one step away from becoming member of Myfuel.co.in.  We're glad that you found us.<br><br>
					Please enter below OTP to verify your account:<br>
					Your OTP : ".$random_no."
				</p>
				<br>
				<p>Note: This OTP is valid only 1 hour, after that the user can not verify his/her account using this otp.</p>
				</p>Support Team,</p>
				<p>".email_from_name."</p>
				<br>
	...
	{
		$data = array('page_title'=>"MyFuel | Verify Account", 'active'=>'profile' );
		$this->load->view('frontend/verify_account',$data);
	}
	...
	function send_password_recovery_link(){
		...
		if($this->form_validation->run()==false){
			...
			if($profile == ''){
				...
				redirect(base_url('frontend/forgot_password'));
			}else{
				...
				$reset_link=base_url('frontend/reset_password/?random_no='.$random_no);
	...
	function reset_password(){
		...
		if(!empty($_REQUEST['random_no'])){
			...
			if($rec > 0 ){
				...
				$this->load->view('frontend/reset-pwd',$data);
			}else{
	...
	function update_reset_password(){
		...
		if($this->form_validation->run()==false){
			...
			redirect(base_url('frontend/reset_password?random_no='.$_POST['random_no']));
	...
	{
		$data = array('page_title'=>"MyFuel | About", 'active'=>'about' );
		$this->load->view('frontend/about',$data);
	}

	public function contact_us(){
		$data = array('page_title'=>"MyFuel | Contact Us", 'active'=>'contact' );
	...
	{
		$data = array('page_title'=>"MyFuel | Privacy Policy", 'active'=>'home' );
		$this->load->view('frontend/privacy-policy',$data);
	}
	
	public function terms_conditions()
	{
		$data = array('page_title'=>"MyFuel | Terms & Conditions", 'active'=>'home' );
		$this->load->view('frontend/terms-conditions',$data);
	}
	
	public function sitemap(){
		$data = array('page_title'=>"MyFuel | Sitemap", 'active'=>'home' );

		$this->load->view('frontend/sitemap',$data);
	}
	
	public function help(){
		$data = array('page_title'=>"MyFuel | Help", 'active'=>'home' );
	...
	public function blogs()
	{
		...
		$config = array();
        $config["base_url"] = base_url() . "frontend/blogs";
	...
	public function blog_details($id=0)
	{
		...
		$data = array('page_title'=>"MyFuel | Blogs", 
			'active'=>'blogs',
			'blogs'=>$blogs,
	...
	public function shop_category(){
		...
		$data['page_title']="MyFuel | Shop";
		$data['active']= 'shop';
		$this->load->view('frontend/shop_category',$data);
	}

	public function shop_curtain(){

		$data['page_title']="MyFuel | Cutrains";
		$data['active']= 'shop';
		$this->load->view('frontend/shop_category_curtains',$data);
	}
	public function shop_wallpaper(){

		$data['page_title']="MyFuel | Wallpaper";
		$data['active']= 'shop';
		$this->load->view('frontend/shop_category_wallpaper',$data);
	}
	public function shop_flooring(){

		$data['page_title']="MyFuel | Flooring";
		$data['active']= 'shop';
		$this->load->view('frontend/shop_category_flooring',$data);
	}
	public function shop_cushions(){

		$data['page_title']="MyFuel | Cushions";
		$data['active']= 'shop';
		$this->load->view('frontend/shop_category_cushions',$data);
	}
	public function shop_mattress(){

		$data['page_title']="MyFuel | Mattress";
		$data['active']= 'shop';
		$this->load->view('frontend/shop_category_mattress',$data);
	}
	public function shop_carpets(){

		$data['page_title']="MyFuel | Carpet";
		$data['active']= 'shop';
		$this->load->view('frontend/shop_category_carpet',$data);
	}
	
	public function shop_bed_linen(){

		$data['page_title']="MyFuel | Bed Linen";
		$data['active']= 'shop';
		$this->load->view('frontend/shop_category_bed_linen',$data);
	}
	public function shop_bathroom_linen(){

		$data['page_title']="MyFuel | Batroom Linen";
		$data['active']= 'shop';
		$this->load->view('frontend/shop_category_bathroom_linen',$data);
	}

	public function shop_blinds(){

		$data['page_title']="MyFuel | Blind";
		$data['active']= 'shop';
		$this->load->view('frontend/shop_category_blinds',$data);
	}


	public function shop_grass(){

		$data['page_title']="MyFuel | Artificial Garden and Grass";
		$data['active']= 'shop';
		$this->load->view('frontend/shop_category_grass',$data);
	}
	...
	public function products($cat_id=''){
		...
		$cat_id = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
		$brand_id = ($this->uri->segment(4)) ? $this->uri->segment(4) : 0;
	    $scat_id = ($this->uri->segment(5)) ? $this->uri->segment(5) : 0;
		...
		$where = array('status' => 1, 'main_category' =>$cat_id, 'free_flag' => 0);
		}
		$group_by = '';
		$order_by = 'position';
		$order = 'DESC';
		...
		$table = 'product_category';
		$where = array('status_id' => 1,'id'=>$cat_id);
		$single_category = $this->user_model->get_common($table, $where,'*',2);
		...
		$table1 = 'products';
		if($cat_id=="" || $cat_id==1 ){
		$where1= array('status' => 1, 'free_flag' => 0);
		}
		else{
		$where1= array('status' => 1, 'main_category' =>$cat_id, 'free_flag' => 0);
		}
		
		$config = array();
        $config["base_url"] = base_url() . "frontend/products/".$cat_id.'/'.$brand_id.'/'.$scat_id;
        $config["total_rows"] = $this->user_model->record_count($table1,$where1);
        $config["per_page"] = 9;
        ...
        $this->pagination->initialize($config);
		$page = ($this->uri->segment(6)) ? $this->uri->segment(6) : 0;
		$product_page = $this->user_model->get_common($table1, $where1,'*',2, $config["per_page"], '', 'position', 'DESC', $page);
		
		$str_links = $this->pagination->create_links();
		$links= explode('&nbsp;',$str_links);
		//print_r($product_page);exit();
		...
		$data['page_title']= 'MyFuel | Products';
		$data['active']= 'shop';
	...
	public function filter_products($cat_id='',$sub_cat='',$fev_id='',$page_id=''){
       error_reporting(0);
		...
		$config = array();
        $config["base_url"] = base_url() . "frontend/filter_products/".$cat_id.'/'.$brand_id.'/'.$sub_cat;
        $config["total_rows"] = $this->user_model->record_count($table,$where);
        $config["per_page"] = 9;
        $config["uri_segment"] = 6;
		$config['num_tag_open'] = '<li class="item-pagination flex-c-m trans-0-4">';
	...
	public function product_category(){
		...
		$data['page_title']="MyFuel | Product Category";
		$table = 'product_category';
		$where = array('status_id' => 1);
		$category = $this->user_model->get_common($table, $where,'*',2);
	...
	public function view_product_details(){
		...
		$data['active']= 'shop';
		$this->load->view('frontend/view_product_details',$data);
	}
	...
	public function view_product_details1(){
		...
		$this->load->view('frontend/view_product_details1',$data);
	}
	...
	public function tracking(){
		$data = array('page_title'=>"MyFuel | Tracking", 'active'=>'track' );
		$this->load->view('frontend/add_track',$data);
	}
	...
	public function cart(){
		...
		$data = array('page_title'=>"MyFuel | My Cart", 'active'=>'cart', 'tbl_cart'=>$tbl_cart );
	...
	public function profile_cart(){
		...
		$this->load->view('frontend/profile_cart',$data);
	}
	...
	public function profile_wishlist(){
		...
		$data = array('page_title'=>"MyFuel | My Cart", 'active'=>'cart', 'tbl_wishlist'=>$tbl_wishlist );

		$this->load->view('frontend/profile_wishlist',$data);
	}
	...
	public function services(){
		...
		$data = array('page_title'=>"MyFuel | Services", 'active'=>'services' );
	...
	public function profile(){
		...
		$order_by = 'id';
		$order1 = '';
		$user_profile = $this->user_model->get_common($table, $where,'*',2, '', $group_by, $order_by, $order1);
		...
		$order_by = 'id';
		$order1 = '';
		$user_foundation = $this->user_model->get_common($table, $where,'*',2, '', $group_by, $order_by, $order1);
		...
		$order_by = 'id';
		$order1 = '';
		$user_caste = $this->user_model->get_common($table, $where,'*',2, '', $group_by, $order_by, $order1);

		$data = array('page_title'=>"MyFuel | My Profile" , 'user_profile'=>$user_profile);
	...
	function edit_user_profile(){
		...
		$group_by = '';
		$order_by = 'id';
		$user_profile = $this->user_model->get_common($table, $where,'*',1, '', $group_by, $order_by);
		$data = array( 'user_profile'=>$user_profile);
		$this->load->view('frontend/edit_user_profile', $data);
	}
	...
	public function my_orders(){
		...
		$data = array('page_title'=>"MyFuel | Authenticate", 'active'=>'authenticate', 'order'=> $orders);
	...
	public function family_profile(){
		...
		$data = array('page_title'=>"MyFuel | myorder", 'active'=>'myorder', 'order'=> $orders);
		$this->load->view('frontend/my_order',$data);
	}
	...
	public function view_order_details(){
		...
		$addr_id=$order[0]->address_flag;
		$data = array(
			'page_title' => 'Order Details | MyFuel',
			'order' => $order,
			'addr_id' => $addr_id,
			'products' => $products
		);
		
		$this->load->view('frontend/view_order_details', $data);
	}
	...
	public function order_payment_enquiry(){
		...
		if(count($payment_data)>0){
			...
			$this->load->view('frontend/HostedPaymentEnquiry',$data);
		}else{
	...
	public function cancel_delete_order_refund(){
		...
		if(count($payment_data)>0){
			...
			$this->load->view('frontend/HostedPaymentRefund',$data);
		}else{
	...
	public function authenticate(){
		...
		$data = array('page_title'=>"MyFuel | Authenticate", 'active'=>'authenticate' );
		$this->load->view('frontend/authenticate',$data);
	}
	...
	public function save_comment(){
    	...
    	if($this->form_validation->run()==false){
			...
			redirect(base_url('frontend/blog_details/'.$_POST['id']));
    	}
	}
	...
	{
		$data = array('page_title'=>"MyFuel | Verify Request", 'active'=>'profile' );
		$this->load->view('frontend/verify_request',$data);
	}
	...
	public function talk_with_experts(){
		...
		if($_SESSION['guest_user']){
			...
			$data = array('page_title'=>"MyFuel | Talk to an Expert", 'guest_user'=>$guest_user, 'chat'=>$chat);
		}else{
			$data = array('page_title'=>"MyFuel | Talk to an Expert");
		}
		
		$this->load->view('frontend/talk_with_experts', $data);
	}
	...
	public function show_topic(){
		...
		$this->load->view('frontend/previous_conversation', $data);
	}
	...
	public function send_message(){
    	...
    	if($this->form_validation->run()==false){
			...
			*/
			...
			$data = array('page_title'=>"MyFuel | Talk to an Experts", 'guest_user'=>$guest_user, 'chat'=>$chat);
			$this->load->view('frontend/previous_conversation', $data);
	...
	public function ajaxsend_message(){
		...
		$data = array('page_title'=>"MyFuel | Talk to an Experts", 'guest_user'=>$guest_user, 'chat'=>$chat);
		$this->load->view('frontend/ajaxsend_message', $data);
	...
	public function save_enquiry(){
	   ...
	   //$prevpage = $_SERVER['HTTP_REFERER'];
		...
		if($this->form_validation->run()==false ){
			...
			//redirect(base_url('frontend/contact_us'));
			$this->contact_us();
		}else{
	...
	public function add_to_card(){
		...
		$this->load->view('frontend/update_header_cart',$data);
	}
	...
	public function remove_from_card(){
		...
		$this->load->view('frontend/update_cart',$data);
	}
	...
	public function apply_coupon(){
	   ...
	   if(count($code1)>0){
		...
		$data = array('page_title'=>"MyFuel | My Cart", 'active'=>'cart', 'tbl_cart'=>$tbl_cart, 'coupon_code'=>$coupon_code, 'coupon_discount'=>$coupon_discount );
		$this->load->view('frontend/update_cart',$data);  
	...
	public function apply_coupon_checkout(){
	   ...
	   if(count($code1)>0){
		...
		$data = array('page_title'=>"MyFuel | My Cart", 'active'=>'cart', 'tbl_cart'=>$tbl_cart, 'coupon_code'=>$coupon_code, 'coupon_discount'=>$coupon_discount );
		$this->load->view('frontend/update_checkout',$data);  
	...
	public function checkout(){
		...
		$data = array('page_title'=>"MyFuel | Check Out", 'active'=>'Check Out','coupon_code'=>$coupon_code,'coupon_discount'=>$coupon_discount,'tbl_cart'=>$tbl_cart );
	...
	public function payu_checkout($cust_addr_id){
		ob_start();
		$data = array('page_title'=>"MyFuel | Check Out", 'active'=>'Check Out','cust_addr_id'=>$cust_addr_id);
		$this->load->view('frontend/HostedPaymentdetailsHttp',$data);
	}
	...
	public function process_order(){
		...
		if($payment_type == 'online')
		{
			...
			//$data = array('page_title'=>"MyFuel | Check Out", 'active'=>'Check Out');
			//$data['menu'] = $this->load->view('frontend/checkout', NULL, TRUE);
			//$this->load->view('frontend/payu_checkout',$data); 
			...
			$this->session->set_flashdata("success_message","Your Order Placed Suuessfully!.");
				...
				$this->load->view('frontend/order_receipt', $data);
	...
	public function guest_user(){
		...
		$data = array('page_title'=>"MyFuel | Guest User", 'active'=>'guest_user' );
		$this->load->view('frontend/guest_user',$data);
