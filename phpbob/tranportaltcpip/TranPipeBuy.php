<HTML>
	...
	<?php 
		...
		try {
			...
			if(trim($myObj->performTransaction())!=0)  {
				...
				header("Location: http://localhost/myfuel/frontend/refund_success/0/0/0");
			}else{
				$payment_status = $myObj->getResult();
				$txt_id = $myObj->getTransId();
				$track_id = $myObj->getTrackId();
				header("Location: http://localhost/myfuel/frontend/refund_success/$payment_status/$txt_id/$track_id");
			?>
			<BODY class="bg">
				<br>
	...
	<?php 
		...
		<TABLE align=center>
			<tr>
				<td> <FONT size=2 color="BLUE"><A href="http://myfuel.co.in/my-orders/">Back</A> </FONT> </td>
			</tr>
		</table>
</HTML>