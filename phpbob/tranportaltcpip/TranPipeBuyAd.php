<HTML>
	...
	<?php 
		...
		try {
			...
			if(trim($myObj->performTransaction())!=0)  {
				...
				//header("Location: http://localhost/myfuel/admin/refund_success/0/0/0");
			}else{
				$payment_status = $myObj->getResult();
				$txt_id = $myObj->getTransId();
				$track_id = $myObj->getTrackId();
				$old_track = $comment;
				header("Location: https://myfuel.co.in/admin/refund_success/$payment_status/$txt_id/$track_id/$old_track");
			?>
			<BODY class="bg">
				<br>
	...
	<?php 
		...
		<TABLE align=center>
			<tr>
				<td> <FONT size=2 color="BLUE"><A href="https://myfuel.co.in/admin/order/">Back</A> </FONT> </td>
			</tr>
		</table>
</HTML>